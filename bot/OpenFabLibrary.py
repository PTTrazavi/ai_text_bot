...
def JeibaCutWords(input_df):
    ...
    # 設定字典
    #jieba.set_dictionary(os.path.join(os.path.dirname(os.path.realpath(__file__)), '../Jeiba/dict.txt.big')) #GCP
    jieba.set_dictionary('./Jeiba/dict.txt.big')
    #jieba.load_userdict(os.path.join(os.path.dirname(os.path.realpath(__file__)), '../Jeiba/my.dict.txt')) #GCP
    jieba.load_userdict('./Jeiba/my.dict.txt')  # 載入自訂字典

    # 設定停用詞
    #with open((os.path.join(os.path.dirname(os.path.realpath(__file__)), '../Jeiba/stop_words.txt')), 'r', encoding='utf8') as f: #GCP
...
def AppendKeywordCheck(input_df):
    ...
    for index, row in input_df.iterrows():  # 每一則廣告
        ...
        for token in enumerate(row['sentence']):  # 每一個詞
            #f = open((os.path.join(os.path.dirname(os.path.realpath(__file__)), '../Jeiba/illegal.keywords.txt')), 'r', encoding='UTF-8')  # GCP
